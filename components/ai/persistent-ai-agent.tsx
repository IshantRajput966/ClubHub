...
export default function PersistentAIAgent() {
  ...
  // Hide on home / login / matchmaker pages (redundant on matchmaker)
  const hidden = pathname === "/" || pathname.startsWith("/login") || pathname === "/matchmaker"
        ...
        <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-0">
          ...
          {!showTeaser && (
            <button
              ...
              className="w-12 h-12 rounded-full bg-gradient-to-br from-purple-500 to-violet-700 shadow-lg shadow-purple-900/50 flex items-center justify-center border border-purple-400/40 hover:scale-110 transition-transform duration-300 hover:shadow-purple-500/40 hover:shadow-xl relative animate-float"
              aria-label="Open AI assistant"
            >
              <span className="absolute w-12 h-12 rounded-full border border-purple-400/30 animate-ping-slow" />
              <svg viewBox="0 0 24 24" fill="none" className="w-6 h-6 text-white" stroke="currentColor" strokeWidth={1.5}>

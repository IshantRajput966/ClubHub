...
function StatCard({ icon: Icon, label, value, sub, color }: {
  ...
  return (
    <div className="group relative p-1 rounded-[31px] bg-gradient-to-b from-white/10 to-transparent transition-all hover:scale-[1.02] shadow-2xl">
      <div className="h-full w-full rounded-[30px] bg-black/40 backdrop-blur-3xl p-6 border border-white/5 flex items-center gap-5 shadow-inner">
        <div className={`w-14 h-14 rounded-2xl flex items-center justify-center shrink-0 shadow-lg group-hover:scale-110 transition-transform ${color}`} style={{ background: "rgba(255,255,255,0.05)" }}>
          <Icon size={24} />
        </div>
        <div>
          <p className="text-3xl font-black text-white tracking-tighter uppercase italic">{value}</p>
          <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mt-1">{label}</p>
          {sub && <p className="text-[9px] text-purple-400 font-bold uppercase tracking-widest mt-0.5">{sub}</p>}
        </div>
      </div>
    </div>
  )
...
function SectionHeader({ icon: Icon, title, color }: { icon: any, title: string, color: string }) {
  return (
    <div className={`flex items-center gap-3 mb-6`}>
      <div className={`w-10 h-10 rounded-xl flex items-center justify-center border border-white/5 shadow-lg ${color}`} style={{ background: "rgba(255,255,255,0.03)" }}>
        <Icon size={18} />
      </div>
  ...
  if (role !== "faculty") {
    return (
      <div className="relative flex h-screen w-full overflow-hidden text-white">
        <Sidebar />
        <div className="flex-1 flex items-center justify-center flex-col gap-6 z-0">
          ...
          <div className="text-center">
            <h2 className="text-2xl font-black text-white uppercase tracking-tighter italic">Signal Restricted</h2>
            <p className="text-gray-500 font-bold text-[10px] uppercase tracking-[0.2em] mt-2">Faculty Neural Auth Required</p>
          </div>
          <button onClick={() => window.location.href = "/dashboard"} className="h-10 px-6 rounded-xl bg-white/[0.03] border border-white/10 text-gray-400 font-bold text-[10px] uppercase tracking-widest hover:bg-white/[0.08] transition-all">
            Return to Dashboard
          </button>
        </div>
      </div>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="flex items-center justify-between mb-12">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-3xl font-black tracking-tighter text-white uppercase italic">Neural Command Reports</h1>
                <p className="text-[10px] text-gray-500 font-bold uppercase tracking-[0.2em] mt-1 italic">IIST Indore • Advanced Campus Intelligence Protocol</p>
              </div>
            </div>
            <div className="flex items-center gap-2 px-4 py-2 bg-white/[0.03] border border-white/5 rounded-2xl">
              <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
              <span className="text-[9px] text-gray-400 font-black uppercase tracking-widest">REAL-TIME DATASTREAM</span>
            </div>
          </div>
          ...
          {/* ── Club Activity ── */}
          <div className="rounded-[31px] border border-white/5 p-10 mb-8 shadow-2xl transition-all" style={{ background: "rgba(255,255,255,0.03)", backdropFilter: "blur(24px)" }}>
            <div className="flex items-center justify-between mb-10 pb-6 border-b border-white/5">
              <SectionHeader icon={BookOpen} title="Entity Activity Matrix" color="text-purple-400" />
              <div className="flex p-0.5 bg-white/[0.03] border border-white/5 rounded-xl">
                <button
                  onClick={() => { setSortClub("members"); setClubAsc(sortClub === "members" ? !clubAsc : false) }}
                  className={`px-4 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all ${sortClub === "members" ? "bg-gradient-to-r from-purple-600 to-blue-600 text-white shadow-lg" : "text-gray-500 hover:text-white"}`}
                >
                  MEMBERS {sortClub === "members" && (clubAsc ? <ChevronUp size={10} className="inline ml-1" /> : <ChevronDown size={10} className="inline ml-1" />)}
                </button>
                <button
                  onClick={() => { setSortClub("events"); setClubAsc(sortClub === "events" ? !clubAsc : false) }}
                  className={`px-4 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all ${sortClub === "events" ? "bg-gradient-to-r from-purple-600 to-blue-600 text-white shadow-lg" : "text-gray-500 hover:text-white"}`}
                >
                  EVENTS {sortClub === "events" && (clubAsc ? <ChevronUp size={10} className="inline ml-1" /> : <ChevronDown size={10} className="inline ml-1" />)}
                </button>
              </div>
            </div>
              ...
              {sortedClubs.map(club => (
                <div key={club.id} className="flex items-center gap-4 p-3 rounded-xl hover:bg-white/5 transition">
                  ...
                  <div className="flex-1 space-y-1.5">
                    <div className="flex items-center gap-4">
                      <span className="text-[10px] text-gray-600 font-bold uppercase tracking-widest w-16">Populus</span>
                      <div className="flex-1 h-3 bg-white/[0.03] border border-white/5 rounded-full overflow-hidden shadow-inner">
                        <div className="h-full bg-gradient-to-r from-purple-600 to-blue-600 rounded-full transition-all duration-1000" style={{ width: `${(club.memberCount / maxMembers) * 100}%` }} />
                      </div>
                      <span className="text-sm font-black text-white w-8 text-right italic">{club.memberCount}</span>
                    </div>
                    <div className="flex items-center gap-4">
                      <span className="text-[10px] text-gray-600 font-bold uppercase tracking-widest w-16">Temporal</span>
                      <div className="flex-1 h-3 bg-white/[0.03] border border-white/5 rounded-full overflow-hidden shadow-inner">
                        <div className="h-full bg-gradient-to-r from-blue-600 to-cyan-600 rounded-full transition-all duration-1000 delay-100" style={{ width: `${(club.eventCount / maxEvents) * 100}%` }} />
                      </div>
                      <span className="text-sm font-black text-white w-8 text-right italic">{club.eventCount}</span>
                    </div>
                  </div>
                </div>
              ))}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <div className="rounded-[31px] border border-white/5 p-8 shadow-2xl" style={{ background: "rgba(255,255,255,0.03)", backdropFilter: "blur(24px)" }}>
              <SectionHeader icon={Users} title="Global Role Distribution" color="text-blue-400" />
              <div className="space-y-3">
            ...
            {/* Domain breakdown */}
            <div className="rounded-[31px] border border-white/5 p-8 shadow-2xl" style={{ background: "rgba(255,255,255,0.03)", backdropFilter: "blur(24px)" }}>
              <SectionHeader icon={Globe} title="Domain Stratification" color="text-orange-400" />
              <div className="space-y-3">

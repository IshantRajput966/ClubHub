...
function MiniCalendar({ events, onSelectDate, selectedDate }: {
  ...
  return (
    <div className="rounded-[31px] border border-white/5 p-5 shadow-2xl" style={{ background: "rgba(255,255,255,0.03)", backdropFilter: "blur(12px)" }}>
      {/* Month nav */}
      <div className="flex items-center justify-between mb-6 px-1">
        <button onClick={prevMonth} className="p-2 rounded-xl hover:bg-white/10 text-gray-400 hover:text-white transition-all">
          <ChevronLeft size={16} />
        </button>
        <p className="text-sm font-black text-white uppercase tracking-widest">{monthName}</p>
        <button onClick={nextMonth} className="p-2 rounded-xl hover:bg-white/10 text-gray-400 hover:text-white transition-all">
...
function UpcomingList({ events, onSelectDate }: { events: Event[], onSelectDate: (d: string) => void }) {
  ...
  return (
    <div className="rounded-[31px] border border-white/5 p-5 mt-6 shadow-2xl" style={{ background: "rgba(255,255,255,0.03)", backdropFilter: "blur(12px)" }}>
      <p className="text-[10px] font-black text-gray-500 uppercase tracking-[0.2em] mb-4">Queue</p>
      {upcoming.length === 0 ? (
          ...
          <div className="max-w-3xl mx-auto p-4 md:p-10">
            ...
            <div className="flex items-center justify-between mb-10">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-3xl font-black tracking-tighter text-white uppercase italic">Neural Calendar</h1>
                  {selectedLabel ? (
                    <p className="text-[10px] text-purple-400 font-bold uppercase tracking-widest mt-1">Filtering: {selectedLabel}</p>
                  ) : (
                    <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest mt-1">Synchronized Campus Events</p>
                  )}
                </div>
              </div>
              {canCreate && (
        ...
        <div className="hidden lg:block w-96 shrink-0 border-l border-white/5 overflow-y-auto p-6 scrollbar-hide"
          ...
          <div className="mb-6">
            <p className="text-[10px] font-black text-gray-500 uppercase tracking-[0.2em] mb-4 flex items-center gap-2">
              <Calendar size={12} /> Temporal Nexus
            </p>
            <MiniCalendar
              events={events}
